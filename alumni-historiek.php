...
// ---------------------------------------------------------------------------
// WP Activity Log (WSAL) integration
// Logs a custom event whenever the public /historiek page is visited.
// The event is only fired when WSAL is active; the plugin works normally
// without it.
// ---------------------------------------------------------------------------

/**
 * Register the custom WSAL alert for the /historiek page visit.
 * Event ID 6100 is used (6000-6999 range is reserved for third-party plugins).
 */
add_filter('wsal_custom_alerts', function (array $alerts): array {
    $alerts[6100] = [
        6100,
        200, // WSAL_INFORMATIONAL
        __('Historiek page visited', 'alumni-historiek'),
        __('The public historiek page (/historiek) was visited by %ClientIP%.', 'alumni-historiek'),
    ];
    return $alerts;
});

/**
 * Fire the WSAL event when /historiek is served.
 * Hooked on 'wp' so the query has been set up but no output has been sent yet.
 */
add_action('wp', function (): void {
    if (!alumni_historiek_is_public_request()) {
        return;
    }
    ...
    foreach (['HTTP_CLIENT_IP', 'HTTP_X_FORWARDED_FOR', 'REMOTE_ADDR'] as $key) {
        ...
        if ($val !== '') {
            // X_FORWARDED_FOR may be a comma-separated list; use the first entry.
            $client_ip = trim(explode(',', $val)[0]);
            break;
        }
    }

    WpSecurityAuditLog::triggerEvent(6100, [
        '%ClientIP%' => $client_ip,
        '%URI%'      => '/historiek',
    ]);
});
